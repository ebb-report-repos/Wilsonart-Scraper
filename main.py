...
def clean_and_export(df_compare: pd.DataFrame, both_available: pd.DataFrame, output_dir="output"):
    # --- clean sizes and fill missing ---
    for df in [df_compare, both_available]:
        ...
        df["SizeDescription_LA"] = df["SizeDescription_LA"].fillna(df["SizeDescription"])
        df["SizeDescription_SA"] = df["SizeDescription_SA"].fillna(df["SizeDescription"])
        df["Size_LA"] = df["Size_LA"].fillna(df["Size"])
        df["Size_SA"] = df["Size_SA"].fillna(df["Size"])

    # --- base columns + dynamic arrival dates ---
    base_cols = [
        'PartNumber','VendPartNumber', 'DesignID', 'DesignName_LA', 'Grade', 'FinishID','SizeDescription', 'Size',
...
if st.button("Export Excel"):
    try:
        file_path = clean_and_export(df_compare, both_available)
        st.success(f"Exported successfully! File saved at: {file_path}")
        st.download_button(
            "Download Excel File",
